...
//	if (isEmpty(slot)) return;
	...
	for (CompEntry::const_iterator i = components.begin(); i != components.end(); ++i) {
        ...
        printf("end bad stuff \n");
	}
delete_all:
    printf("start delete_all\n");
	std::string save2;
	for (CompEntry::const_iterator i = components.begin(); i != components.end(); ++i) {
		save2=temp+i->first;
        printf("deleting %s\n", save2.c_str());
    ...
    printf("end delete_all\n");
    printf("finished loading state\n");
}
...
extern bool saveStateRequested;
extern bool loadStateRequested;
