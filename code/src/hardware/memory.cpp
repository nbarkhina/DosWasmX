...
unsigned char pagehandler_idx[10000];
...
private:
	virtual void getBytes(std::ostream& stream)
	{
        ...
        // FILE* file = fopen("mem.txt", "wb");        // w for write, b for binary
        // fwrite(pagehandler_idx, neilMaxSize, 1, file); // write 2048 bytes
        // fclose(file);
	...
	virtual void setBytes(std::istream& stream)
	{
        ...
        // FILE* f = fopen("mem.txt", "rb");
        // fseek(f, 0, SEEK_END);
        // int fsize = ftell(f);
        // fseek(f, 0, SEEK_SET);  /* same as rewind(f); */
        // fread(pagehandler_idx, 1, fsize, f);
        // fclose(f);
