...
//NEIL - this gets called about 48 times a second
static void SDLCALL MIXER_CallBack(void * userdata, Uint8 *stream, int len) {
    ...
    if (!mixer.prebuffer_wait && !mixer.mute) {
        ...
        while (need > 0) {
            ...
            resampled_out_buf[neilAudioWritePosition] = MIXER_CLIP((((int64_t)(*in++)) * (int64_t)volscale1) >> (MIXER_VOLSHIFT + MIXER_VOLSHIFT));
            ...
            *output++ = 0;
            ...
            resampled_out_buf[neilAudioWritePosition] = MIXER_CLIP((((int64_t)(*in++)) * (int64_t)volscale2) >> (MIXER_VOLSHIFT + MIXER_VOLSHIFT));
            neilAudioWritePosition++;
            if (neilAudioWritePosition == 64000)
            {
                neilAudioWritePosition = 0;
...
void MIXER_Init() {
    ...
    if (mixer.nosound) {
        ...
        printf("audio obtained channels %d frequency %d\n", obtained.channels, obtained.freq);
