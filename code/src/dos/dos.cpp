...
#endif
    if (GetCurrentDirectory(512, (LPWSTR)winDirCur)&&(net||!strncmp(Drives[drive]->GetInfo(),"local ",6)||!strncmp(Drives[drive]->GetInfo(),"CDRom ",6))) {
        ...
        if (!strncmp(Drives[DOS_GetDefaultDrive()]->GetInfo(),"local ",6)||!strncmp(Drives[DOS_GetDefaultDrive()]->GetInfo(),"CDRom ",6)) {
            ...
            if (!PathFileExists((LPWSTR)winDirNew)) {
                bool olfn=uselfn;
                uselfn=true;
        ...
        if (SetCurrentDirectory((LPWSTR)winDirNew)||net) {
            ...
            DWORD temp = (DWORD)SHGetFileInfo((LPWSTR)winName,0,NULL,0,SHGFI_EXETYPE);
            ...
            SetCurrentDirectory((LPWSTR)winDirCur);
            if (startwait && hret > 0) {
                int count=0;
                ctrlbrk=false;
                DWORD exitCode = 0;
