...
#include "../builtin/cdplay.h"
#include "../builtin/zip.h"
#include "../builtin/mpxplay.h"
#include "../builtin/ne2000.h"
#include "../builtin/textutil.h"
#include "../builtin/4DOS_img.h"
#include "../builtin/emsmagic.h"
#include "../builtin/eval.h"
#include "../builtin/shutdown.h"
#include "../builtin/glide2x.h"
#include "../builtin/dosmid.h"
...
public:
    ...
    void Run(void) {
        ...
        //NEIL - don't boot if we can't find windows directory
        if (drive == 'C')
        {
	...
	private:
		...
		bool MountIso(const char drive, const std::vector<std::string> &paths, const signed char ide_index, const bool ide_slave) {
            ...
            //print root files
            {
                ...
                bool ret = DOS_FindFirst(path_search, 0xffff), found = true, first = true;
                if (ret) {
                    //std::string win95 = std::string("WIN95");
                    //std::string win98 = std::string("WIN98");
                    ...
                    do {    /* File name and extension */
                        ...
                        //if (win95 == std::string(result.name) || win98 == std::string(result.name))
                        //{
                        //    printf("detected windows iso\n");
                        //    foundWindows = true;
                        //}

                        printf("iso mounted root file: %s\n", result.name);
...
public:
    void Run() {
        ...
        if (GetCurrentDirectory(512, (LPSTR)winDirCur)&&(!strncmp(Drives[DOS_GetDefaultDrive()]->GetInfo(),"local ",6)||!strncmp(Drives[DOS_GetDefaultDrive()]->GetInfo(),"CDRom ",6))) {
            Overlay_Drive *ddp = dynamic_cast<Overlay_Drive*>(Drives[DOS_GetDefaultDrive()]);
            ...
            if (SetCurrentDirectory((LPCSTR)winDirNew)) setdir=true;
        }
        if (!startquiet) WriteOut("Starting %s...\n", cmd);
        ShellExecuteEx(&lpExecInfo);
        int ErrorCode = GetLastError();
        if (setdir) SetCurrentDirectory((LPCSTR)winDirCur);
...
#if defined(WIN32) && !defined(__EMSCRIPTEN__) || defined(LINUX) || defined(MACOSX)
...
void DOS_SetupPrograms(void) {
    ...
    MSG_Add("PROGRAM_INTRO",
        "\033[2J\033[32;1mWelcome to DosWasm-X\033[0m, an open-source x86 emulator with sound and graphics.\n"
        "DosWasm-X creates a shell for you which looks just like the plain DOS.\n"
        "\n"
        "\033[31;1mDosWasm-X will stop/exit without a warning if an error occurred!\033[0m\n"
        "\n"
        "\n" );
