...
void drivezRegister(std::string const& path, std::string const& dir, bool usecp) {
    ...
    if (path.size()) {
        ...
        //        } else if ((host_name == NULL || temp_name != NULL) && strcmp(host_name != NULL ? temp_name : (const char*)fd.cFileName, ".") && strcmp(host_name != NULL ? temp_name : (const char*)fd.cFileName, ".."))
        //            names.push_back(std::string(host_name == NULL ? (const char*)fd.cFileName : temp_name)+"/");
...
unsigned long localDrive::GetCompressedSize(char* name) {
	...
	CROSS_FILENAME(newname);
	dirCache.ExpandName(newname);
	DWORD size = GetCompressedFileSize((LPCSTR)newname, NULL);
	if (size != INVALID_FILE_SIZE) {
		if (size != 0 && size == GetFileSize(newname, NULL)) {
			DWORD sectors_per_cluster, bytes_per_sector, free_clusters, total_clusters;
			if (GetDiskFreeSpace((LPCSTR)newname, &sectors_per_cluster, &bytes_per_sector, &free_clusters, &total_clusters)) {
...
HANDLE localDrive::CreateOpenFile(const char* name) {
	...
	CROSS_FILENAME(newname);
	dirCache.ExpandName(newname);
	HANDLE handle=CreateFile((LPCSTR)newname, FILE_WRITE_ATTRIBUTES, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
...
unsigned long localDrive::GetSerial() {
	...
	if (strlen(newname)>2&&newname[1]==':') {
		...
		char volume[] = "A:\\";
		volume[0]=newname[0];
		GetVolumeInformation((LPCSTR)volume, NULL, 0, &serial_number, NULL, NULL, NULL, 0);
	...
	*_free_clusters=allocation.free_clusters;
	if ((!allocation.total_clusters && !allocation.free_clusters) || freesizecap) {
        ...
        if (basedir[0]=='\\' && basedir[1]=='\\')
            res = GetDiskFreeSpace((LPCSTR)basedir, &dwSectPerClust, &dwBytesPerSect, &dwFreeClusters, &dwTotalClusters);
        else
            res = GetDiskFreeSpace(drive? (LPCSTR)root:NULL, &dwSectPerClust, &dwBytesPerSect, &dwFreeClusters, &dwTotalClusters);
		if (res) {
            ...
            for (int tries = file_access_tries; tries; tries--) {
                if (::LockFile(hFile, pos, 0, size, 0)) {
                    if (lockWarn && ::LockFile(hFile, pos, 0, size, 0)) {
                        ...
                        MessageBox(NULL, (LPCSTR)"Record locking seems incorrectly implemented!\nConsult ...", 
                            (LPCSTR)caption, MB_OK|MB_ICONSTOP);
                    }
                    return true;
                }
